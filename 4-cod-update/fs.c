...
/////////////////////creare
struct inode create_inode(char* name,int i_parent,uint32_t mode, uint32_t uid, uint32_t gid, uint32_t blocks)
{
    struct inode i;
    i.i_mode=mode;
    i.i_uid=uid;
...
void create_root()
{
    Inodes[0]=create_inode("root",0,777,0,0,0);
    SB.nr_free_inodes--;
    SB.i_bitmap[0]=1;

    for(int i=1;i<MAX_INODES;i++)
       Inodes[i]=create_inode("",0,0,0,0,0);
}
//////////////////////////
